...
static void initializeIoTHubClient()
{
    ...
    if (az_result_failed(az_iot_hub_client_get_user_name(
            &client, mqtt_username, sizeofarray(mqtt_username), nullptr)))
    {
 ...
 */
static uint32_t getEpochTimeInSecs() { return (uint32_t)time(nullptr); }
...
/////////////////// [START] ACTUAL CODE TO SEND SENSOR DATA ///////////////////////
static void generateTelemetryPayload()
{
    JsonDocument sensorData;

    std::mt19937 mtEngine {static_cast<std::mt19937::result_type>(
                                   std::chrono::steady_clock::now().time_since_epoch().count()
                           )};
    std::uniform_real_distribution<double> tempRandom {34, 42};
    std::uniform_real_distribution<double> gaitRandom {-200, 200};
    std::uniform_real_distribution<double> dilationRandom {0, 7};




    sensorData["head_temperature"] = tempRandom(mtEngine);
    sensorData["z_gait_left"] = gaitRandom(mtEngine);
    sensorData["z_gait_right"] = gaitRandom(mtEngine);
    sensorData["dilation_diameter"] = dilationRandom(mtEngine);

    sensorData["min_dilation_diameter"] = 0;
    sensorData["max_dilation_diameter"] = 7;
    ...
    if (frameBuffer == nullptr) {
        ...
        JsonArray data = sensorData["camera_data"].to<JsonArray>();
        for (int i{}; i < frameBuffer->len; ++i) {
            data.add(frameBuffer->buf[i]);
        }
    }
    esp_camera_fb_return(frameBuffer);

    Serial.println("Payload created");
    serializeJson(sensorData, telemetry_payload);
...
static void sendTelemetry()
{
    ...
    if (az_result_failed(az_iot_hub_client_telemetry_get_publish_topic(
            &client, nullptr, telemetry_topic, sizeof(telemetry_topic), NULL)))
    {
...
/////////////////// [END] ACTUAL CODE TO SEND SENSOR DATA ///////////////////////
...
// Obtained from stackoverflow by DrBeco
